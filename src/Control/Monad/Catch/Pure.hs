...
instance (Monad m
         ...
         , Total m
#endif
         ) => MonadThrow (CatchT m) where
  throwM = CatchT . return . Left . toException
instance (Monad m
#if MIN_VERSION_base(4,14,0)
         , Total m
#endif
         ) => MonadCatch (CatchT m) where
